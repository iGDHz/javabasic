...
import com.hz.InputFromater.ExampleInputFormat;
...
public class MapReduceExample extends Configured implements Tool {
    public static class WeiBoMapper extends Mapper<Text, WeiBo, Text, Text>{
        @Override
        protected void map(Text key, WeiBo value, Mapper.Context context) throws IOException, InterruptedException{
    ...
    public int run(String[] args) throws Exception{
        ...
        job.setJarByClass(MapReduceExample.class);// 主类
        ...
        job.setInputFormatClass(ExampleInputFormat.class);// 自定义输入格式
        //自定义文件输出类别
...
//                "hdfs://HadoopMaster:9000/out/weibo/" };

        String[] args0 = { "/data/weibo/weibo.txt",
                "/out/weibo/" };

        int ec = ToolRunner.run(new Configuration(), new MapReduceExample(), args0);
        System.exit(ec);
    }
}

...
public class HzInputFormat extends FileInputFormat<Text, Order> {
    ...
    public RecordReader<Text, Order> createRecordReader(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException{
        ...
        return new OrderRecordReader();//新建一个ScoreRecordReader实例，所有才有了上面RecordReader<Text,ScoreWritable>，所以才如下ScoreRecordReader，写我们自己的
    }
    ...
    public class OrderRecordReader extends RecordReader<Text, Order>{
        ...
        public boolean nextKeyValue() throws IOException, InterruptedException{//这里面，才是篡改的重点
            ...
            String price = null;
            try{
                productid = Integer.parseInt(pieces[1].trim());//商品id
                price = pieces[2];
            ...
            lineValue.set(productid,price);//完成自定义value数据
            //              或者写
